...
use Mouf\Database\DBConnection\Column;

use Mouf\Database\DBConnection\Table;
 ...
 *
 * @ExtendedAction {"name":"Generate stat table", "url":"dbStatsAdmin/", "default":false}
 * @ExtendedAction {"name":"Recompute stat table", "url":"dbStatsAdmin/recomputeForm", "default":false}
 */
class DB_Stats implements MoufValidatorInterface {
	...
	/**
	 * 
	 * @param ConnectionInterface $dbConnection The connection to the database that contains the table we are working on.
	 ...
	 */
	public function __construct(ConnectionInterface $dbConnection, $sourceTable, $statsTable, array $dimensions = array(), array $values = array()){
	...
	public function createStatsTable($dropIfExist = false) {
		...
		foreach ($this->dimensions as $dimension) {
			foreach ($dimension->getColumns() as $column) {
				...
				if (isset($columnsList[$column->getColumnName()])) {
					continue;
				}
				$columnsList[$column->getColumnName()] = 1;
				
				$dbColumn = new Column();
				$dbColumn->name = $column->getColumnName();
				$dbColumn->type = $column->getType();
		...
		foreach ($dimensionCombinations as $dimensionCombination) {
			...
			foreach ($dimensionCombination as $dimension) {
				foreach ($dimension->getColumns() as $column) {
					$columnsArr[] = $column->getColumnName();
				}
			}
	...
	private function onInsertSqlStatement($columns, $filterColumns, $columnsArrArr, $valuesArr) {
		...
		foreach ($this->values as $value) {
			$updateStrArr[] = $value->getColumnName()."=".$value->getColumnName()."+".$value->getDataOriginForInsertTrigger();
		}
	...
	private function onDeleteSqlStatement($columns, $filterColumns, $columnsArrArr, $valuesArr) {
		...
		$updateStrArr = array();
		foreach ($this->values as $value) {
			$updateStrArr[] = $value->getColumnName()."=".$value->getColumnName()."-".$value->getDataOriginForUpdateTrigger();
		}
		$sql .= implode(", ", $updateStrArr);
		$sql .= " WHERE ";
		...
		$zeroValuesArr = array();
		foreach ($this->values as $value) {
			$zeroValuesArr[] = $value->getColumnName()." = 0";
		}
	...
	private function getNullableColumnsCombinations() {
		...
		foreach ($this->dimensions as $dimension) {
			...
			$dimensionColumnsArray[] = array();
			foreach ($dimension->getColumns() as $column) {
				$dimensionColumnsWork[] = $column->getColumnName();
	...
	public function getFilterColumns() {
		...
		foreach ($this->dimensions as $dimension) {
			foreach ($dimension->getColumns() as $column) {
				$filterColumns[$column->getColumnName()] = $column; 
			}
		}
	...
	private function getValuesColumns() {
		...
		foreach ($this->values as $value) {
			$valuesArr[$value->getColumnName()] = $value;
		}
		return $valuesArr;
	}
	 ...
	 *
	 * @param array<DB_Dimension> $dimensions
	 */
